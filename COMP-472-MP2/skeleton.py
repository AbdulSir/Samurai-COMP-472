...
class Game:
    ...
    def play(self):
        # file_name = "gameTrace-<" + str(self.gp.size_of_board) + '><' + str(self.gp.num_of_blocs) + '><' + str(self.gp.line_up_size) + '><' + str(self.gp.threshold) + '>.txt'
        # while True:
        ...
        if (self.player_turn == 'X' and self.mode_of_play()[0] == self.HUMAN) or (
                self.player_turn == 'O' and self.mode_of_play()[1] == self.HUMAN):
            if self.recommend:
                # with open(file_name, "a") as myfile:
                # 	myfile.write(F'\nEvaluation time: {round(end - self.start, 7)}s')
                # 	myfile.write(F'\nRecommended move: x = {x}, y = {y}')
                print(F'Evaluation time: {round(end - self.start, 7)}s')
                print(F'Recommended move: x = {x}, y = {y}')
            (x, y) = self.input_move()
        if (self.player_turn == 'X' and self.mode_of_play()[0] == self.AI) or (
                self.player_turn == 'O' and self.mode_of_play()[1] == self.AI):
            print(F'Evaluation time: {round(end - self.start, 7)}s')
